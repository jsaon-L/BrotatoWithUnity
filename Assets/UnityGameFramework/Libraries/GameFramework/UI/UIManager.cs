...
namespace GameFramework.UI
{
    ...
    internal sealed partial class UIManager : GameFrameworkModule, IUIManager
    {
        ...
        public void CloseUIForm(IUIForm uiForm, object userData)
        {
            ...
            //ui 控制补充数据
            CloseUIFormParam uIControlAvg = userData as CloseUIFormParam;

            uiGroup.RemoveUIForm(uiForm);

            if (uIControlAvg == null)
            {
                //关闭后刷新
            ...
            {
                //关闭后不刷新
                uiForm.OnClose(m_IsShutdown, uIControlAvg.UserData);
                if (uIControlAvg.RefreshUI)
                {
                    uiGroup.Refresh();
                }
                ReferencePool.Release(uIControlAvg);
            }

            if (m_CloseUIFormCompleteEventHandler != null)
            {
                CloseUIFormCompleteEventArgs closeUIFormCompleteEventArgs = CloseUIFormCompleteEventArgs.Create(uiForm.SerialId, uiForm.UIFormAssetName, uiGroup, uIControlAvg == null ? userData : uIControlAvg.UserData);
