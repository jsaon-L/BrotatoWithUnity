    ...
    public class ListView : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IEventSystemHandler
    {
        ...
        public void SetInitData(Dictionary<string, GameObject> prefabDic, FuncTab funcObj)
        {
            ...
            foreach (var prefab in prefabDic)
            {
                ...
                {
                    GameFramework.GameFrameworkLog.Warning("ListViewInit data name is null!");
                    continue;
                }
                ...
                text = prefab.Key;
                assetObjectDic[text] = prefabDic[text];
                Rect rect = prefabDic[text].GetComponent<RectTransform>().rect;
            ...
            if (autoPlaySound)
            {
                ...
                if(delta>0&&delta<1)
                {
                    if (Mathf.Abs(delta - m_lastSoundDelta) >= soundInterval/itemList.Count)
                    {
                        ...
                        m_audioHandler = UISoundManager.PlaySound(soundAssetName);
                    }
                }
            }
        }
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            UISoundManager.StopSound(m_audioHandler);
            if (this.parentScrollRect != null && this.parentScrollEnable)
            {
