...
using static StarForce.ProcedureGameStart;
...
namespace StarForce
{
    public class NetworkChannelHelper : INetworkChannelHelper
    {
        ...
        public bool Serialize<T>(T packet, Stream destination) where T : Packet
        {
            ...
            //得到数据包长度
            //var length = (int)m_CachedStream.Length;
            var length = 4;

      
            
            //开始组装[长度+id+数据包]
            BufferWriter buffer = ReferencePool.Acquire<BufferWriter>();
            buffer.WriteInt(length + 4);
            buffer.WriteInt(packet.Id);
            buffer.WriteInt((packet as MyPacket).Name);

            m_CachedStream.Position = 0L;
            //buffer.WriteFrom(m_CachedStream);
            m_CachedStream.Flush();


            buffer.WriteTo(destination);
            ReferencePool.Release(buffer);
            UnityEngine.Debug.LogError(destination.Length.ToString());
            return true;
        }
        ...
        public IPacketHeader DeserializePacketHeader(Stream source, out object customErrorData)
        {
            ...
            BufferReader buffer = ReferencePool.Acquire<BufferReader>();
            buffer.WriteFrom(source);

            //从流中先读取4字节作为长度，在读取2字节作为Id，手动赋值PacketHeader
            var header = ReferencePool.Acquire<SCPacketHeader>();
            header.PacketLength = buffer.ReadInt() - 4;
        ...
        public Packet DeserializePacket(IPacketHeader packetHeader, Stream source, out object customErrorData)
        {
            ...
            UnityEngine.Debug.LogError("DeserializePacket:"+source.Length);
            UnityEngine.Debug.LogError("DeserializePacket"+source.Position);
            //Type packetType = GetServerToClientPacketType(scPacketHeader.Id);
            MyPacket packet = ReferencePool.Acquire< MyPacket>();


            BufferReader buffer = ReferencePool.Acquire<BufferReader>();
            buffer.WriteFrom(source);

            packet.Name = buffer.ReadInt();
